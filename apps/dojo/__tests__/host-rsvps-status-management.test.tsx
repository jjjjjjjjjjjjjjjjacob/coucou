...
type MockRsvpWithCustomFields = {
  customFieldValues?: Record<string, string>;
};

// Mock the Convex hooks
const mockApprove = mock((_args: MockMutationArgs) =>
  Promise.resolve({ ok: true, code: "ABC123" }),
);
const mockDeny = mock((_args: MockMutationArgs) => Promise.resolve({ ok: true }));
const mockToggleRedemptionStatus = mock((_args: MockMutationArgs) =>
  Promise.resolve({ status: "enabled" }),
);
const mockUseQuery = mock((api: string, _args: MockMutationArgs) => {
        ...
        <table>
          ...
          <tbody>
            ...
            <tr data-testid="rsvp-pending">
              ...
              <td>{/* No ticket actions for pending */}</td>
            </tr>
            {/* Approved RSVP */}
              ...
              <td>
                ...
                <div data-testid="status-options-approved" style={{ display: "none" }}>
                  <div
                    ...
                    onClick={() => {
                      /* Would need a mutation to set to pending */
                    }}
                  >
                    Pending
                  </div>
...
describe("RSVP Status Management", () => {
  ...
  describe("Approval Status Changes", () => {
    it("should approve a pending RSVP and create a ticket", async () => {
      ...
      const approveOption = pendingRows[0].querySelector(
        '[data-testid="status-option-approved"]',
      ) as HTMLElement;
    ...
    it("should deny a pending RSVP", async () => {
      ...
      // Get all pending rows and use the first one
      const pendingRows = screen.getAllByTestId("rsvp-pending");
      const denyOption = pendingRows[0].querySelector(
        '[data-testid="status-option-denied"]',
      ) as HTMLElement;
    ...
    it("should re-approve a denied RSVP and re-enable ticket", async () => {
      ...
      const approveOption = deniedRows[0].querySelector(
        '[data-testid="status-option-approved"]',
      ) as HTMLElement;
      fireEvent.click(approveOption);
  ...
  describe("Ticket Status Changes", () => {
    it("should toggle ticket status for approved RSVP", async () => {
      ...
      await waitFor(() => {
        expect(mockToggleRedemptionStatus).toHaveBeenCalledWith({
          rsvpId: "rsvp2",
        });
      });
...
describe("Custom Field Column Generation", () => {
  it("should generate columns for all custom fields", () => {
    ...
    const customFieldColumns = mockCustomFields.map((field) => ({
      ...
      header: field.label,
      accessorFn: (r: MockRsvpWithCustomFields) => r.customFieldValues?.[field.key] || "",
    }));
  ...
  it("should handle missing custom fields gracefully", () => {
    ...
    const accessorFn = (r: MockRsvpWithCustomFields) => r.customFieldValues?.["title"] || "";
  ...
  it("should handle completely missing custom fields", () => {
    ...
    const accessorFn = (r: MockRsvpWithCustomFields) => r.customFieldValues?.["company"] || "";
    const result = accessorFn(mockRsvp);
    ...
    const filterByApproval = (
      ...
      return rsvps.filter((rsvp) => {
        const approvalStatus =
          rsvp.status === "approved" || rsvp.status === "attending" ? "approved" : rsvp.status;
...
describe("Advanced RSVP Management Features", () => {
  ...
  const mockUpdateRsvpComplete = mock((_args: MockMutationArgs) =>
  ...
  const MockAdvancedRSVPManagement = () => {
    ...
    const hasChanges = (rsvpId: string) => {
      ...
      return (
        changes.originalApprovalStatus !== changes.currentApprovalStatus ||
              ...
              {mockRsvpData.map((rsvp) => {
                ...
                return (
                  <tr
                    ...
                    style={{
                      backgroundColor: rowHasChanges ? "#fefce8" : undefined,
                    }}
                  >
                    <td>{rsvp.name}</td>
                    <td>
                      <div data-testid={`advanced-approval-dropdown-${rsvp.id}`}>
                        ...
                        <div
                          data-testid={`advanced-approval-menu-${rsvp.id}`}
                          style={{ display: "none" }}
                        >
                          <button
                      ...
                      <div data-testid={`advanced-ticket-dropdown-${rsvp.id}`}>
                        ...
                        >
                          {currentTicketStatus === "none"
                            ? "Not issued"
                            : currentTicketStatus.charAt(0).toUpperCase() +
                              currentTicketStatus.slice(1)}
                        </button>
                        <div
                          data-testid={`advanced-ticket-menu-${rsvp.id}`}
                          style={{ display: "none" }}
                        >
                          <button
