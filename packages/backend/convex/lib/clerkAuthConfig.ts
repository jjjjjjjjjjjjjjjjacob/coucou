...
export function resolveClerkFrontendApiUrls(environmentVariables: {
  ...
  const primaryFrontendApiUrls = splitFrontendApiUrls(environmentVariables.CLERK_FRONTEND_API_URL);
  const additionalFrontendApiUrls = splitFrontendApiUrls(
    environmentVariables.CLERK_FRONTEND_API_URLS,
  );
  const frontendApiUrls = [...primaryFrontendApiUrls, ...additionalFrontendApiUrls];
...
export function buildClerkAuthProviders(environmentVariables: {
  ...
  return resolveClerkFrontendApiUrls(environmentVariables).map((frontendApiUrl) => ({
