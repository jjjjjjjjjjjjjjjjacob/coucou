...
describe("auth domain helpers", () => {
  it("lists configured client site redirect origins", () => {
    expect(getClientSiteRedirectOrigins()).toContain(
      "https://dojopomodoro.club",
    );
    expect(getClientSiteRedirectOrigins()).toContain(
      "https://clubchlorine.party",
    );
    expect(getClientSiteRedirectOrigins()).not.toContain(
      "https://coucou.events",
    );
  });
  ...
  it("builds a satellite return URL that triggers Clerk session sync", () => {
    const returnUrl = new URL(
      buildSatelliteReturnUrl(
        "https://dojopomodoro.club",
        "/events/event_123/ticket?source=menu",
      ),
    );
    ...
    expect(returnUrl.searchParams.get(CLERK_SATELLITE_SYNC_PARAM)).toBe(
      "false",
    );
  });
